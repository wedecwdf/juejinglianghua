...
def initialize_next_day_adjustment(adj_ctx: NextDayAdjustmentContext,
                                   prev_close_price: float,
                                   config: Condition2Config) -> None:
    if not config.next_day_adjustment_enabled:
        return
    ...
    if c2_activated:
        ...
        sell_ratio += (config.sell_percent_high if inc >= config.dynamic_line_threshold else config.sell_percent_low)
    if c9_activated:
        inc = (c9_high - prev_close_price) / prev_close_price
        sell_ratio += (config.sell_percent_high if inc >= config.dynamic_line_threshold else config.sell_percent_low)

    sell_ratio = min(sell_ratio, config.next_day_max_sell_ratio)

    days_count = data.get("days_count", 0) + 1
    if days_count > config.next_day_max_days:
        data["enabled"] = False
        logger.info("【次日调整机制】达到最大延续天数%d，机制终止", config.next_day_max_days)
    else:
        data.update({
            "enabled": True,
        ...
        logger.info("【次日调整机制】初始化完成 止损价:%.4f 卖出比例:%.1f%% 天数:%d/%d",
                     stop_loss_price, sell_ratio * 100, days_count, config.next_day_max_days)


def check_dynamic_profit_next_day_adjustment(adj_ctx: NextDayAdjustmentContext,
                                             current_price: float,
                                             available_position: int,
                                             config: Condition2Config) -> Optional[int]:
    if not config.next_day_adjustment_enabled:
        return None
    ...
    trigger_price = stop_loss_price - config.next_day_stop_loss_offset
...
def update_dynamic_profit_high_lines(adj_ctx: NextDayAdjustmentContext,
                                     condition_type: str,
                                     current_profit_line: float,
                                     config: Condition2Config) -> None:
    if not config.next_day_adjustment_enabled:
        return
...
def disable_next_day_adjustment_if_dynamic_profit_triggered(adj_ctx: NextDayAdjustmentContext,
                                                            condition_type: str,
                                                            config: Condition2Config) -> None:
    if not config.next_day_adjustment_enabled:
        return
