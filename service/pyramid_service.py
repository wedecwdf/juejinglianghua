...
# -*- coding: utf-8 -*-
"""
动态回调加仓服务，所有配置通过 config 参数传入，无模块级可变状态。
"""
...
# 不再有模块级 _callback_config 常量


def should_create_callback_task(condition_type: str, config: CallbackAddConfig) -> bool:
    if not config.enabled:
        return False
    # 不允许为板相关条件创建任务
    if condition_type in ['board_dynamic_profit', 'board_break', 'board_break_static', 'board_break_dynamic']:
        return False
    if condition_type in ['condition2', 'condition9', 'condition8', 'condition8_pyramid_profit']:
        return True
    return False
...
def add_callback_task(symbol: str, sell_price: float, prev_close: float,
                      ...
                      config: CallbackAddConfig) -> Optional[CallbackAdditionTask]:
    ...
    if old_task_data and old_task_data.get('is_active'):
        ...
        logger.info("【动态回调加仓-任务覆盖】%s 旧任务被强制废止: "
                     ...
                     symbol, old_task.sell_price, old_task.callback_threshold * 100,
                     old_task.condition_type)
    ...
    if task.buy_quantity < config.min_trade_unit:
        logger.info("【动态回调加仓】%s 折算股数%d低于最小单位%d，任务自动终止。"
                     "卖出金额=%.2f, 触发价=%.4f",
                     symbol, task.buy_quantity, config.min_trade_unit, sell_amount,
                     task.trigger_price)
...
def check_callback_strategy(symbol: str, current_price: float,
                            store: AbstractCallbackTaskStore,
                            config: CallbackAddConfig) -> Optional[Dict[str, Any]]:
    if not config.enabled:
        return None
    ...
    if task.is_triggered(current_price):
        logger.info("【动态回调加仓-触发】%s 当前价=%.4f <= 触发价=%.4f, "
                     ...
                     task.callback_threshold * 100, task.buy_quantity,
                     task.condition_type)
        return {
