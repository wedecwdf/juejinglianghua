...
from config.strategy.config_objects import Condition2Config, Condition9Config, Condition8Config, MaTradingConfig
...
def check_condition4(day_data: DayData, context: Condition4To7Context,
                     current_price: float, config: MaTradingConfig) -> Optional[Dict[str, Any]]:
    return _cond4(day_data, context, current_price, config)


def check_condition5(day_data: DayData, context: Condition4To7Context,
                     current_price: float, config: MaTradingConfig) -> Optional[Dict[str, Any]]:
    return _cond5(day_data, context, current_price, config)


def check_condition6(day_data: DayData, context: Condition4To7Context,
                     current_price: float, config: MaTradingConfig) -> Optional[Dict[str, Any]]:
    return _cond6(day_data, context, current_price, config)


def check_condition7(day_data: DayData, context: Condition4To7Context,
                     current_price: float, tick_time, config: MaTradingConfig) -> Optional[Dict[str, Any]]:
    return _cond7(day_data, context, current_price, tick_time, config)
