...
def handle_market_close(symbol: str, tick_time: datetime,
    ...
    logger.info("条件互斥状态: %s",
                '条件2已触发，禁止条件9' if ctx2.condition2_triggered_and_sold else '正常')
    ...
    if PYRAMID_PROFIT_ENABLED:
        ...
        logger.info("【独立机制】金字塔止盈: 已触发级别 %s", triggered_levels if triggered_levels else '无')
        logger.info(" 股票类别: %s", stock_type)
        logger.info(" 该股票独立配置的总卖出数量: %d股", total_qty)
