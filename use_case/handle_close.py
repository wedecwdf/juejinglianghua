...
def handle_market_close(symbol: str, tick_time: datetime,
    ...
    # 条件4-7状态
    ctx47 = session_registry.get_condition4_7(symbol)
    print(f"条件4买入: {'已触发' if ctx47.buy_condition4_triggered else '未触发'}")
    print(f"条件5买入: {'已触发' if ctx47.buy_condition5_triggered else '未触发'}")
    print(f"条件6买入: {'已触发' if ctx47.buy_condition6_triggered else '未触发'}")
    print(f"条件7卖出: {'已触发' if ctx47.condition7_triggered else '未触发'}")

    # 条件8状态
    ctx8 = session_registry.get_condition8(symbol, day_data.base_price)
    print(f"条件8交易次数: {ctx8.condition8_trade_times}/10")
    print(f"条件8状态: {'休眠' if ctx8.condition8_sleeping else '活跃'}")

    # 条件2状态
    ctx2 = session_registry.get_condition2(symbol)
    print(f"条件2动态止盈: {'已触发' if ctx2.dynamic_profit_triggered else '未触发'}, "
          f"卖出次数: {ctx2.dynamic_profit_sell_times}")

    # 条件9状态
    ctx9 = session_registry.get_condition9(symbol, day_data.base_price)
    print(f"条件9动态止盈: {'已触发' if ctx9.condition9_triggered else '未触发'}, "
          f"卖出次数: {ctx9.condition9_sell_times}")
    print(f"条件9停止监测: {'是' if ctx9.condition9_stopped else '否'}")
    print(f"条件互斥状态: {'条件2已触发，禁止条件9' if ctx2.condition2_triggered_and_sold else '正常'}")

    # 板数打印
    bcd = board_repo.get_board_count_data(symbol)
    if bcd:
        print(f"板数: 第{bcd.count}板 首板日期:{bcd.start_date} "
              f"昨收:{bcd.prev_close:.2f} 涨停价:{bcd.limit_up_price:.2f}")
    else:
        print("板数: 未涨停")
    ...
    if CALLBACK_ADDITION_ENABLED:
        ...
        if task_data:
            ...
            print(f" 任务详情: 卖出价={task_data.get('sell_price', 0):.4f}, "
                  f"昨收={task_data.get('prev_close', 0):.4f}, "
                  f"获利幅度={task_data.get('callback_threshold', 0)*100:.2f}%, "
                  f"触发价={task_data.get('trigger_price', 0):.4f}, "
                  f"计划买入={task_data.get('buy_quantity', 0)}股, "
    ...
    if PYRAMID_PROFIT_ENABLED:
        pyr_ctx = session_registry.get_pyramid(symbol, day_data.base_price)
        status = pyr_ctx.pyramid_profit_status
        triggered_levels = [i + 1 for i, s in enumerate(status) if s]
        total_qty = PYRAMID_TOTAL_QUANTITY.get(symbol, 0)
        stock_type = "高频" if symbol in HIGH_FREQUENCY_STOCKS else (
            "低频" if symbol in LOW_FREQUENCY_STOCKS else "默认")
        ...
        print(f" 独立基准价: {pyr_ctx.pyramid_profit_base_price:.4f}")
        print(f" 独立触发状态: {'已触发' if pyr_ctx.pyramid_profit_triggered else '未触发'}")
    else:
        print("金字塔止盈: 已禁用")
    ...
    if DYNAMIC_PROFIT_NEXT_DAY_ADJUSTMENT_ENABLED:
        ...
        if adj_ctx.data.get("enabled", False):
            ...
            print(f" 止损价: {adj_ctx.data.get('stop_loss_price', 0):.4f}")
            print(f" 卖出比例: {adj_ctx.data.get('sell_ratio', 0)*100}%")
            print(f" 延续天数: {adj_ctx.data.get('days_count', 0)}")
        else:
